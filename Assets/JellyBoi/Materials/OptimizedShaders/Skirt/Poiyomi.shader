Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/33e1f36afd7a5544fac537efd6893661"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			{
				#if defined(PROP_HEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
				float heightMask = POI2D_SAMPLER_PAN(_Heightmask, _linear_repeat, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				if (float(0))
				{
			...
			void applyParallax(inout PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam)
			{
				...
				poiMesh.uv[float(0)] = POM(poiLight, _HeightMap, poiMesh, poiCam.viewDir, poiCam.tangentViewDir, float(10), float(128), float(0.005), 0, float4(1,1,0,0).xy, float2(float(0), float(0)));
				#else
				float2 offset = POM(poiLight, _HeightMap, poiMesh, poiCam.viewDir, poiCam.tangentViewDir, float(10), float(128), float(0.005), 0, float4(1,1,0,0).xy, float2(float(0), float(0)));
